...
from crf_from_prob import dense_crf


dir = '40_CRF'

if not os.path.exists(os.path.join(config.RESULT_DIR, "FCN_EXT", dir)):
    os.makedirs(os.path.join(config.RESULT_DIR, "FCN_EXT", dir))
#
# #%%
# print("LOAD DATA")
...
with tf.variable_scope("conv5", reuse=tf.AUTO_REUSE):
    ...
    conv5_3_W = tf.get_variable("conv5_3_W", [3, 3, 1024, 1024], initializer=tf.contrib.layers.xavier_initializer())
    conv5_3_b = tf.get_variable("conv5_3_b", [1024], initializer=tf.zeros_initializer())
...
with tf.variable_scope("conv6", reuse=tf.AUTO_REUSE):
    conv6_1_W = tf.get_variable("conv6_1_W", [3, 3, 1024, 1024], initializer=tf.contrib.layers.xavier_initializer())
    conv6_1_b = tf.get_variable("conv6_1_b", [1024], initializer=tf.zeros_initializer())
    conv6_2_W = tf.get_variable("conv6_2_W", [3, 3, 1024, 1024], initializer=tf.contrib.layers.xavier_initializer())
    conv6_2_b = tf.get_variable("conv6_2_b", [1024], initializer=tf.zeros_initializer())
    conv6_3_W = tf.get_variable("conv6_3_W", [3, 3, 1024, 1024], initializer=tf.contrib.layers.xavier_initializer())
    conv6_3_b = tf.get_variable("conv6_3_b", [1024], initializer=tf.zeros_initializer())
conv6_1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(maxpool5, conv6_1_W, strides=[1, 1, 1, 1], padding="SAME"), conv6_1_b))
conv6_2 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv6_1, conv6_2_W, strides=[1, 1, 1, 1], padding="SAME"), conv6_2_b))
conv6_3 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(conv6_2, conv6_3_W, strides=[1, 1, 1, 1], padding="SAME"), conv6_3_b))

maxpool6 = tf.nn.max_pool(conv6_3, ksize = [1,2,2,1], strides=[1, 2, 2, 1], padding="SAME")

with tf.variable_scope("fc6", reuse=tf.AUTO_REUSE):
    fc6_W = tf.get_variable("fc6_W", [7, 7, 1024, 4096], initializer=tf.contrib.layers.xavier_initializer())
    fc6_b = tf.get_variable("fc6_b", [4096], initializer=tf.zeros_initializer())
fc6 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(maxpool6, fc6_W, strides=[1, 1, 1, 1], padding="SAME"), fc6_b))
...
Ufc7 = tf.nn.conv2d_transpose(fc8, Ufc7_W, tf.stack([tf.shape(maxpool5)[0], tf.shape(maxpool5)[1], tf.shape(maxpool5)[2], config.classes]), strides=[1, 2, 2, 1], padding="SAME")

with tf.variable_scope("Spool4", reuse=tf.AUTO_REUSE):
    Spool4_W = tf.get_variable("Spool4_W", [1, 1, 1024, config.classes], initializer=tf.contrib.layers.xavier_initializer())
    Spool4_b = tf.get_variable("Spool4_b", [config.classes], initializer=tf.zeros_initializer())
Spool4 = tf.nn.bias_add(tf.nn.conv2d(maxpool5, Spool4_W, strides=[1, 1, 1, 1], padding="SAME"), Spool4_b)
...
Ufuse1 = tf.nn.conv2d_transpose(fuse1, Ufuse1_W, tf.stack([tf.shape(maxpool4)[0], tf.shape(maxpool4)[1], tf.shape(maxpool4)[2], config.classes]), strides=[1, 2, 2, 1], padding="SAME")

with tf.variable_scope("Spool3", reuse=tf.AUTO_REUSE):
    Spool3_W = tf.get_variable("Spool3_W", [1, 1, 512, config.classes], initializer=tf.contrib.layers.xavier_initializer())
    Spool3_b = tf.get_variable("Spool3_b", [config.classes], initializer=tf.zeros_initializer())
Spool3 = tf.nn.bias_add(tf.nn.conv2d(maxpool4, Spool3_W, strides=[1, 1, 1, 1], padding="SAME"), Spool3_b)
...
Z = tf.nn.conv2d_transpose(fuse2, Output_W, tf.stack([tf.shape(maxpool5)[0], config.patch_size, config.patch_size, config.classes]), strides=[1, 16, 16, 1], padding="SAME")
...
with tf.Session() as sess:
    saver.restore(sess, os.path.join(config.MODEL_DIR, "FCN_EXT_40", "model.ckpt"))
    ...
    for batch in range(n_batches):
        ...
        crf_output = np.zeros((config.FCN_batch_size,config.patch_size, config.patch_size))
        print('The shape of crf_output', crf_output.shape)

        for i, (image, label) in enumerate(zip(data_batch, output)):
            print('Shape of image', image.shape, 'Shape of label', label.shape)
            mod_label = label
            for i in range(512):
                for j in range(512):
                    if mod_label[i,j] == 0:
                        mod_label[i,j] = 0.001
            crf_output[i] = dense_crf(image.astype('uint8'), mod_label)

        preds[batch*config.FCN_batch_size:(batch+1)*config.FCN_batch_size, :, :] = crf_output
...
for i in range(n_data):
    image = test_data[i,:,:,:]
    label = test_label[i,:,:]
    print("For", i,"th test file", label.shape)
    ...
    plt.tight_layout()
    plt.colorbar()
    plt.savefig(os.path.join(config.RESULT_DIR, "FCN_EXT", dir, str(i)+".png"), format='png')
    print(os.path.join(config.RESULT_DIR, "FCN_EXT", dir, str(i)+".png"))
    plt.close()

...
dir = '50_CRF'
upper_dir = 'FCN'
if not os.path.exists(os.path.join(config.RESULT_DIR, upper_dir, dir)):
    os.makedirs(os.path.join(config.RESULT_DIR, upper_dir, dir))
#
# #%%
# print("LOAD DATA")
#
...
##Uncomment if doing CRF on label
...
with tf.Session() as sess:
    saver.restore(sess, os.path.join(config.MODEL_DIR, "FCN", "model.ckpt"))
    ...
    for batch in range(n_batches):
        ...
        for i, (image, label) in enumerate(zip(data_batch, output)):
            ...
            crf_output[i] = dense_crf(image.astype('uint8'), label)
...
for i in range(n_data):
    ...
    plt.tight_layout()
    plt.colorbar()
    plt.savefig(os.path.join(config.RESULT_DIR, "FCN", str(i)+"iter5.png"), format='png')
    print(os.path.join(config.RESULT_DIR, "FCN", str(i)+"iter5.png"))
    plt.close()
